  ...
  <div class="order-right">
    <div class="order-content">
      ...
      <div class="orders">
        ...
        <table class="order-item" v-for="record in orderData.records" :key="record.id">
          <thead>
            <tr>
              <th colspan="5">
          ...
          <tbody>
            <tr v-for="(detail,index) in record.orderDetailList" :key="detail.id">
              <td :width="index==0 ? '60%':'50%'">
                <div class="typographic">
                  <img :src="detail.imgUrl" style="width:100px;height:100px">
                  <router-link class="block-text" :to="`/detail/${detail.skuId}`">{{detail.skuName}}</router-link>
      ...
      <div class="choose-order">
          <Pagination :pageNo="pageNo" :pageSize="pageSize" :total="orderData.total" :continuous="5" @getPageNo="getPageNo"/>
      </div>
    </div>
    <!--猜你喜欢-->
    <div class="like">
...
export default {
  ...
  methods: {
    // 获取订单数据
    async getMyOrder () {
      let result = await this.$API.reqMyorder(this.pageNo, this.pageSize)
      if (result.code == 200) {
