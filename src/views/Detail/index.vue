    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :zoomImgList="skuInfo.skuDefaultImg"/>
          <!-- 小图列表 -->
          <ImageList :zoomImgList="skuImageList"/>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd :class="{active:spuSaleAttrValue.isChecked == '1'}" v-for="spuSaleAttrValue in spuSaleAttr.spuSaleAttrValueList" 
                  :key="spuSaleAttrValue.id" @click="changeSaleAttr(spuSaleAttr.spuSaleAttrValueList, spuSaleAttrValue)">{{spuSaleAttrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @change="checkSkuNum">
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.$store.dispatch('skuDetail', this.$route.params.skuId)
    },
    methods: {
      changeSaleAttr(spuSaleAttrValueList, spuSaleAttrValue) {
      ...
      async addShopCar() {
        if (this.skuNum !== 0) {
          let result = await this.$store.dispatch('addToCart', {skuId:this.$route.params.skuId, skuNum:this.skuNum})
          if (result == 'success') {
            // 将购物数据用字符串的形式存储到会话存储里
